  ...
  for(Int_t ietak = 0; ietak < 120; ietak++ )
  {
   ...
   for(int i=1;i<73;i++){
    for(int j=1;j<43;j++){
     for(int l=1;l<5;l++){
        k = i*1000+j*10+l;
        sprintf(str0,"if20pl%d",k);
        sprintf(str1,"if20min%d",k);
        sprintf(str2,"if20plcut%d",k);
        sprintf(str3,"if20mincut%d",k);
	
        sprintf(str10,"is20pl%d",k);
        sprintf(str11,"is20min%d",k);
        sprintf(str12,"is20plcut%d",k);
        sprintf(str13,"is20mincut%d",k);
//	cout<<" "<<i<<" "<<j<<" "<<l<<endl;
    if( j < 30 )
    {
// first order moment	
...
// second order moment    
    ...
    hCalo2cutmom2[i][j][l] = new TH1F(str13, "h13", 320, 0., 120.); 
     ...
     for(int j=1;j<43;j++)
     {
       ...
       {
         Int_t jj = 10*j+l;
        sprintf(str4,"if20pleta%d",jj);
        sprintf(str5,"if20mineta%d",jj);
        sprintf(str6,"if20plcuteta%d",jj);
        sprintf(str7,"if20mincuteta%d",jj);
	
        sprintf(str14,"is20pleta%d",jj);
        sprintf(str15,"is20mineta%d",jj);
        sprintf(str16,"is20plcuteta%d",jj);
        sprintf(str17,"is20mincuteta%d",jj);
	
	sprintf(str18,"it20plcuteta%d",jj);
	sprintf(str19,"it20mincuteta%d",jj);
	sprintf(str20,"it20pleta%d",jj);
	sprintf(str21,"it20mineta%d",jj);
	
	sprintf(str22,"ic20pletatwo%d",jj);
	sprintf(str23,"ic20minetatwo%d",jj);
	sprintf(str24,"ic20pletaone%d",jj);
	sprintf(str25,"ic20minetaone%d",jj);
	sprintf(str26,"icd20pletaone%d",jj);
	sprintf(str27,"icd20minetaone%d",jj);
...
//	 cout<<" Point 1"<<endl;
	 ...
	 minmean[abs(ieta)][iphi]=minmean[abs(ieta)][iphi] + (float)disp;
...
//	 cout<<" Point 3"<<endl;
	 ...
	 if( abs(ieta) > 29 ) {
	  ...
	  plmean[ieta][iphi]=plmean[ieta][iphi] + (float)disp;
	  plneveta[abs(ieta)][depth]++;
	  plnevetaphi[iphi][abs(ieta)][depth]++;
	 }
...
//    cout<<" Mean "<<hCalo_cut2eta[1]->GetMean[0]<<endl;
  ...
  for(int ietak = 1; ietak != 42; ietak++ )
  {
    ...
    for(Int_t iphik = 1; iphik != 73; iphik++ )
    {
	 ...
	 if( ietak > 29 )
	 {
	  plmeang[iphik][ietak][idep] = sqrt(plmeang[iphik][ietak][idep]/plmean[ietak][idep]);
	 }
	   else
	   {
                plmeang[iphik][ietak][idep] =plmeang[iphik][ietak][idep]/plmean[ietak][idep];
           }		       
         if(ietak == 1) fprintf(Out1,"%s %d %s %.4f %s\n"," newcor[",iphik,"]=",plmeang[iphik][ietak],";");
         hCalo2etatwocoef[ietak][idep]->Fill((float)iphik,plmeang[iphik][ietak][idep]);
         hCalo2etaonecoef[ietak][idep]->Fill((float)iphik,plmeang[iphik][ietak][idep]);
  ...
  for(int ietak = 1; ietak != 42; ietak++ )
  {
    ...
    for(Int_t iphik = 1; iphik != 73; iphik++ )
    {
	 ...
	 if( ietak > 29 )
	 {
	  minmeang[iphik][ietak][idep] = sqrt(minmeang[iphik][ietak][idep]/minmean[ietak][idep]);
	 }
	   else
	   {
                minmeang[iphik][ietak][idep] =minmeang[iphik][ietak][idep]/minmean[ietak][idep];
           }		       
         if(ietak == 1) fprintf(Out1,"%s %d %s %.4f %s\n"," newcor[",iphik,"]=",minmeang[iphik][ietak],";");
         hCalo1etatwocoef[ietak][idep]->Fill((float)iphik,minmeang[iphik][ietak][idep]);
         hCalo1etaonecoef[ietak][idep]->Fill((float)iphik,minmeang[iphik][ietak][idep]);
	 hCalo1etacoefdist[ietak][idep]->Fill(minmeang[iphik][ietak][idep]);
    }
  }
  }
  ...
  fclose(Out1);
     ...
     TFile efile("mom_20_initial.root","recreate");
